...
class fogNodeCharacterisation(object):
    ...
    def __init__(self, ALPHA=0.2, *args):
        self.G                  = {}
        self.WEIGHTS            = {"PLR": 0.3, "Response": 0.2, "CPU": 0.3, "Accuracy": 0.2}
        self.STATIC_THRESHOLDS  = {"PLR": 10, "Response": 200, "CPU": 80, "Accuracy": 80}
        if not (0 < ALPHA < 1):
        ...
        self.metrics_stats      = {
                                    ...
                                    "Accuracy": {"mu": 90.0, "sigma": 5.0, "n": 0, "sum": 0, "sum_sq": 0}
                                }
        # mu        Historical mean (μ) of the metric. Initial guess or prior. 
        #           PLR=5.0 (5% packet loss), Response=100, CPU, 50.0, Accuracy=90.0
        # sigma     Historical standard deviation (σ). Measures variability. PLR=2.5 (moderate variance)
        # n         Number of observations seen so far. Starts at 0 (no data). PLR,Response,CPU,Accuracy=0
        # sum       Running sum of all observed values. Used to compute mu.  PLR,Response,CPU,Accuracy=0
        # sum_sq    Running sum of squared values. Used to compute sigma.  PLR,Response,CPU,Accuracy=0
    ...
    def _generate_l1_metrics(self) -> Dict:
        ...
        return {
            ...
            "Accuracy": round(random.uniform(87, 94), 2),
            "status": "HEALTHY"
        }
    ...
    def _standard_running_metrics(self, layer: int, num_children: int, failure_count: int, max_failures: int) -> Dict:
        ...
        Returns:
        - Dictionary with PLR, Response, CPU, Accuracy, status, and updated failure_count
        """
        ...
        accuracy = base_accuracy - 0.05 * process_load - 0.03 * forward_load + random.uniform(-1, 1)

        # Clamp and round values
        cpu = min(cpu, 100.0)
        accuracy = max(min(accuracy, 100.0), 80.0)
        plr = max(plr, 0.0)
    ...
    def initialise_metrics_per_node(self, node_metrics) -> Dict:
        ...
        for node, metric in node_metrics.items():
            new_plr = new_response = new_cpu = new_accuracy = None            
            ...
            if node == "CloudDBServer":
                ...
                for metrica in ["PLR", "Response", "CPU", "Accuracy", "status"]: 
                    node_details[metrica] = cloud_node_metric[metrica]       
                #print("node_details: ", node_details)
                
                for key in metric.keys(): 
                    new_plr = metric["PLR"]; new_response = metric["Response"]; new_cpu = metric["CPU"]; new_accuracy = metric["Accuracy"];  node_id = metric["node_id"]
                # Calculate health metrics
                meanAndsd = self.muAndsigma_per_node(node_id, new_plr, new_response, new_cpu, new_accuracy)
                healthmetric = self.healthMetric(
                    new_plr, new_response, new_cpu, new_accuracy, 
                    meanAndsd["PLR"]["mu"], meanAndsd["PLR"]["sigma"],  
                    meanAndsd["Response"]["mu"], meanAndsd["Response"]["sigma"], 
                    meanAndsd["CPU"]["mu"], meanAndsd["CPU"]["sigma"],  
                    meanAndsd["Accuracy"]["mu"], meanAndsd["Accuracy"]["sigma"]
                )
    ...
    def muAndsigma_global(self, new_plr, new_response, new_cpu, new_accuracy) -> Dict:
        ...
        metrics_stats["Accuracy"]["n"] += 1
        metrics_stats["Accuracy"]["sum"] += new_plr
        metrics_stats["Accuracy"]["sum_sq"] += new_plr ** 2

        # Recompute mu and sigma
        n = metrics_stats["Accuracy"]["n"]
        sum_val = metrics_stats["Accuracy"]["sum"]
        sum_sq  = metrics_stats["Accuracy"]["sum_sq"]

        metrics_stats["Accuracy"]["mu"] = sum_val / n 
        mu =  metrics_stats["Accuracy"]["mu"] # µ (mu)
        #metrics_stats["PLR"]["sigma"] = math.sqrt((sum_sq / n) - (sum_val / n) ** 2)
        metrics_stats["Accuracy"]["sigma"] = 0 if abs((sum_sq / n) - mu ** 2) < 1e-10 else math.sqrt((sum_sq / n) - (sum_val / n) ** 2)
        sigma   = metrics_stats["Accuracy"]["sigma"] # σ (sigma) 
        #print("sum_val: ", sum_val, "\n sum_sq: ", sum_sq, "\n n: ", n, "\n sigma", sigma)
        updated_metrics_stats["Accuracy"]["sigma"]   = sigma
        updated_metrics_stats["Accuracy"]["mu"]      = mu
        #time.sleep(1)
        
        return updated_metrics_stats
                
    def muAndsigma_per_node(self, node_id: int, new_plr, new_response, new_cpu, new_accuracy) -> Dict:
        if node_id not in self.metrics_stats:
            ...
            self.metrics_stats[node_id] = {
                ...
                "Accuracy": {"mu": 90.0, "sigma": 5.0, "n": 0, "sum": 0.0, "sum_sq": 0.0}
            }
    ...
    def healthMetric(self, plr, response, cpu, accuracy, 
                 plr_mean, plr_std, response_mean, response_std, 
                 cpu_mean, cpu_std, accuracy_mean, accuracy_std) -> float:
        """
        ...
        z_accuracy  = (accuracy_mean - accuracy) / accuracy_std if accuracy_std != 0 else 0.0
    ...
    def update_metrics_per_node_old(self, node_metrics, update_node_metric):
        ...
        for node, metric in node_metrics.items():
            ...
            if node == "CloudDBServer":
                ...
                #node_details = {"node_id": counter+1}
                cloud_node_metric = self._generate_cloud_metrics()
                for metric in ["PLR", "Response", "CPU", "Accuracy", "status"]: 
    ...
    def update_metrics_per_node(self, processed_metrics: dict, failure_count: int, max_failures: int) -> Tuple[dict, int]:
        ...
        for node_name, details in processed_metrics.items():
            ...
            updated_stats = self.muAndsigma_per_node(node_id, metrics["PLR"], metrics["Response"], metrics["CPU"], metrics["Accuracy"])
            health = self.healthMetric(
                metrics["PLR"], metrics["Response"], metrics["CPU"], metrics["Accuracy"],
    ...
    def healthStatus(self, node_details, healthmetric, adaptive_threshold) -> bool:
        ...
        #["PLR", "Response", "CPU", "Accuracy", "status"]
        plr         = node_details["PLR"]
        response    = node_details["Response"]
        cpu         = node_details["CPU"]
        accuracy    = node_details["Accuracy"]
        
        # = {"PLR": 10, "Response": 200, "CPU": 80, "Accuracy": 80}
        ...
        Accuracy_STATIC_THRESHOLDS  = STATIC_THRESHOLDS["Accuracy"]
        ...
        is_faulty = (
            ...
            cpu > CPU_STATIC_THRESHOLDS or
            accuracy <  Accuracy_STATIC_THRESHOLDS
        )
        if not is_faulty:
            if 1==1:
            #try:
